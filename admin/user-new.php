...
$firstName = '';
$lastName = '';
$roleId = ''; // Default role might be set later
...
// Helper function for file uploads (assuming it exists) - reusing from edit page
if (!function_exists('handleFileUpload')) {
    function handleFileUpload($inputName, $targetDir, $allowedExts, $maxSize, $prefix = '') {
        ...
        if ($_FILES[$inputName]['error'] !== UPLOAD_ERR_OK) { throw new Exception("File upload error for '$inputName'. Code: " . $_FILES[$inputName]['error']); }
        $fileTmpPath = $_FILES[$inputName]['tmp_name']; $fileName = $_FILES[$inputName]['name']; $fileSize = $_FILES[$inputName]['size'];
        $fileInfo = pathinfo($fileName); $extension = strtolower($fileInfo['extension'] ?? '');
        if (!in_array($extension, $allowedExts)) { throw new Exception("Invalid file format for '$inputName'. Allowed: " . implode(', ', $allowedExts)); }
        if ($fileSize > $maxSize) { throw new Exception("File size for '$inputName' too large (Max " . ($maxSize / 1024 / 1024) . "MB)."); }
        $newFilename = $prefix . uniqid() . '.' . $extension;
        if (!is_dir($targetDir)) { if (!mkdir($targetDir, 0755, true)) throw new Exception("Failed to create upload directory: $targetDir"); }
        $uploadPath = rtrim($targetDir, '/') . '/' . $newFilename;
        if (!move_uploaded_file($fileTmpPath, $uploadPath)) { throw new Exception("Failed to move uploaded file for '$inputName'."); }
        return $uploadPath;
    }
}
...
try {
    ...
    // Optionally set a default role ID here if desired
    // if (!empty($roles)) { $roleId = $roles[0]['id']; }

} catch (PDOException $e) {
    $message = "Error loading roles: " . $e->getMessage();
    $messageType = "danger";
    error_log("Roles Load Error (User New Page): " . $e->getMessage());
    // Allow form rendering but role dropdown might be empty
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) throw new Exception("Valid email address is required.");
        ...
        if (empty($roleId)) throw new Exception("User role must be selected.");
        if (empty($status) || !in_array($status, ['active', 'inactive', 'suspended'])) throw new Exception("Invalid user status selected.");

        // Check username uniqueness
        $checkUserStmt = $db->prepare("SELECT COUNT(*) FROM users WHERE LOWER(username) = LOWER(?)");
        ...
        $checkEmailStmt = $db->prepare("SELECT COUNT(*) FROM users WHERE LOWER(email) = LOWER(?)");
        ...
        $profilePicPath = null; // Default to no picture
        $newProfilePicPath = handleFileUpload(
            ...
            'profile_new_' // Filename prefix (use 'new' or similar)
        );
        ...
        $sql = "INSERT INTO users
                    (username, email, password, first_name, last_name, role_id, status, bio, profile_picture, registration_date)
                VALUES
                    (:username, :email, :password, :first_name, :last_name, :role_id, :status, :bio, :profile_picture, NOW())";
        $stmt = $db->prepare($sql);
        ...
        if (function_exists('logActivity')) {
             logActivity($loggedInUser['id'], 'user_create', 'users', $newUserId, "Created new user: '{$username}'");
        }
        ...
        if (isset($profilePicPath) && file_exists($profilePicPath)) {
            @unlink($profilePicPath);
        }
        $message = "Error: " . $e->getMessage();
        $messageType = "danger";
        error_log("User Creation Error: " . $e->getMessage());
        // Form values ($username, $email etc.) are already set for repopulation
    }
}
...
<head>
    ...
    <title>Add New User - <?php echo htmlspecialchars(getSetting('site_name', 'Alpha News')); ?></title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-items">
            ...
            <?php if (hasPermission('manage_categories')): ?>
                ...
                <a href="users.php" class="sidebar-item active" title="Users"> <i class="fas fa-users fa-fw"></i> <span>Users</span> </a>
                <a href="roles.php" class="sidebar-item" title="Roles"> <i class="fas fa-user-tag fa-fw"></i> <span>Roles</span> </a>
            <?php endif; ?>
    ...
    <div class="main-content">
        ...
        <form method="POST" action="user-new.php" id="newUserForm" enctype="multipart/form-data">
             <div class="row">
                <div class="col-lg-8">
                    <div class="card mb-4">
                         ...
                         <div class="card-body">
                             <div class="row">
                                 <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="username" name="username" value="<?php echo htmlspecialchars($username); ?>" required pattern="^[a-zA-Z0-9_]+$" title="Only letters, numbers, and underscores allowed.">
                                    <small class="form-text text-muted">Used for login. Letters, numbers, underscores only.</small>
                                 </div>
                                 <div class="col-md-6 mb-3">
                                     <label for="email" class="form-label">Email Address <span class="text-danger">*</span></label>
                                     <input type="email" class="form-control form-control-sm" id="email" name="email" value="<?php echo htmlspecialchars($email); ?>" required>
                                 </div>
                             </div>
                             <div class="row">
                                 <div class="col-md-6 mb-3">
                                     <label for="first_name" class="form-label">First Name <span class="text-danger">*</span></label>
                                     <input type="text" class="form-control form-control-sm" id="first_name" name="first_name" value="<?php echo htmlspecialchars($firstName); ?>" required>
                                 </div>
                                 <div class="col-md-6 mb-3">
                                     <label for="last_name" class="form-label">Last Name <span class="text-danger">*</span></label>
                                     <input type="text" class="form-control form-control-sm" id="last_name" name="last_name" value="<?php echo htmlspecialchars($lastName); ?>" required>
                                 </div>
                             </div>
                             <div class="row">
                                 <div class="col-md-6 mb-3">
                                 ...
                                 <div class="col-md-6 mb-3">
                                     ...
                                     <input type="password" class="form-control form-control-sm" id="confirm_password" name="confirm_password" required autocomplete="new-password">
                                 </div>
                             </div>
                             <div class="mb-3">
                                <label for="bio" class="form-label">Bio</label>
                                <textarea class="form-control form-control-sm" id="bio" name="bio" rows="3" placeholder="Brief description about the user..."><?php echo htmlspecialchars($bio); ?></textarea>
                            </div>
                         </div>
                    </div>
                </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card mb-4">
                         ...
                         <div class="card-body">
                             <img src="assets/images/default-avatar.png" alt="Profile Picture Preview" class="profile-pic-preview" id="profile_pic_preview">
                     ...
                     <div class="card mb-4">
                        <div class="card-body d-grid gap-2">
                            ...
                            <a href="users.php" class="btn btn-outline-secondary btn-sm"><i class="fas fa-arrow-left me-1"></i> Cancel</a>
                        </div>
                     </div>
            ...
            if (picInput && picPreview) {
                picInput.addEventListener('change', function(e) {
                    ...
                    if (file && file.type.startsWith('image/')) {
                        ...
                        picPreview.src = 'assets/images/default-avatar.png'; // Reset to default if invalid
                    }
                });
            }
